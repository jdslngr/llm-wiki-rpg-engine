...
| 6 | pending | New RecapHistoryScreen.tsx: list (newest-first/chronological sort, empty/loading/error states, summary cards with isFinal/Legacy badges) and detail views (full recap prose, facts, beats, crew, epilogue/acknowledgment, legacy label). RecapHistoryScreen.test.tsx: 11 Vitest + React Testing Library tests covering all states, race condition (delayed-A/fast-B), back-to-game success/failure, sort toggle, detail error. App.tsx: 'recapHistory' screen routing via handleResume. GameScreen.tsx: "Recap History" menu item. client/src/types.ts: RecapSummary, RecapDetailEntry, RecapDetailResponse, RecapListResponse types. vitest.config.ts, test-setup.ts: Vitest + jsdom + @testing-library infrastructure. tsconfig.app.json: exclude test files from main build. | Client build (tsc -b + vite build) clean. All 11 client tests pass. All 5 server verifiers pass (148 tests total: 29+47+22+22+19+17+31). Manual: Recap History accessible from game nav menu. | Proceed to Phase 7. |
