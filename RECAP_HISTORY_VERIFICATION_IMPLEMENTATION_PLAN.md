...
| 5 | pending | Post-review corruption hardening: appends reject corrupt envelopes without modifying raw data; history list/detail return controlled errors instead of legacy fallback. | Pending verification. | Run the complete automated suite, then replace `pending` with the commit hash before handoff. |
