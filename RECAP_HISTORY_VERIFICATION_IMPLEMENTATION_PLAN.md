...
| 2 | 8fd0120 | `server/src/recapArchive.ts`: versioned envelope (`{version:1,entries:[...]}`), centralized `parseArchiveEnvelope()`, exported `archiveEnvelopeError()`, completed `isValidRecapFacts` (crew trust/arc, journey fields, notableFacts), cross-checks in `validateArchiveEntry` (facts.chapterNumber/title vs parent), deep-cloned existing rows in `appendArchivedRecap` (`structuredClone`), v0→v1 upgrade on append. `server/src/recapPreparation.ts`: envelope corruption check, all-rows collection replacing `.find()`. `server/src/verify-recap-history.ts`: +26 tests (facts validation, envelope versioning, deep-clone isolation). `server/src/verify-recap-history-phase3.ts`: +2 tests (valid+duplicate all-rows, corrupt envelope). `server/src/verify-recap-history-routes.ts`: 1 assertion fixed for cross-check change. | Server build ✓, verify-recap-history (73/73, +26) ✓, verify-recap-history-phase3 (25/25, +2) ✓, verify-recap-history-routes (22/22) ✓, client test suite (11/11) ✓ — 131 checks, 0 failures. | `isValidRecapFacts` was exported (was private) so verifiers can test it directly. Unversioned archives are v0 and upgrade to v1 on next append. Corrupt version is not an empty archive — `archiveEnvelopeError()` returns the reason. |
| 3 | cae190f | `server/src/recapArchive.ts`: two-pass legacy parser — collects all candidates, accepts only strictly increasing non-duplicate headings, section boundaries use only accepted headings (rejected ones become prose in prior entry). `RecapSummary.createdAt` made optional; `toSummary` omits it for legacy. `client/src/types.ts`: `RecapSummary.createdAt` and `RecapDetailEntry.createdAt` made optional. `client/src/RecapHistoryScreen.tsx`: `formatDate` accepts `string \| undefined`, returns "Pre-archive save" for absent/invalid timestamps, checks `Number.isNaN(date.getTime())`. | Server build ✓, client build ✓, verify-recap-history (79/79, +6) ✓, verify-recap-history-phase3 (25/25) ✓, verify-recap-history-routes (22/22) ✓, client test suite (13/13, +2) ✓ — 139 checks, 0 failures. | Legacy summaries no longer send `createdAt: ''`. Server verifier has §10 (toSummary timestamp behaviour). Client tests verify "Pre-archive save" renders and "Invalid Date" never appears, including for garbled non-legacy timestamps. |
| 4 | 6e0daac | `server/src/verify-endstate.ts`: fixed smoke-test CWD — resolves `server/` from `import.meta.url`, sets it as child `cwd` (fixes addendum). `README.md`: added versioned archive envelope description, verification commands section with correct `--prefix server` paths, single-process lock limitation noted. `Technical_Specifications.md`: updated recap archive section (v0/v1/corrupt behavior), chapter-end lock (single-process limitation + race fix detail), verifier table (corrected commands, updated test counts). `RECAP_HISTORY_VERIFICATION_IMPLEMENTATION_ADDENDUM.md`: committed (previously untracked). | Server build ✓, client build ✓, verify-recap-history (79/79) ✓, verify-recap-history-phase3 (25/25) ✓, verify-recap-history-routes (22/22) ✓, verify-final-chapter (29/29) ✓, verify-facts (19/19) ✓, verify-facts-recap (17/17) ✓, verify-endstate (31/31, +2 smoke) ✓, verify-store-deletion (Postgres) ✓, client test suite (13/13) ✓, git diff --check ✓ — 235 automated checks, 0 failures. | Browser QA deferred (requires local/staging single-instance app with authenticated browser flow). Postgres `verify-store-deletion.ts` passed against Docker `postgres://archipelago:archipelago@localhost:5432/archipelago`. |
| 5 | e3eda91 | Post-review corruption hardening: appends reject corrupt envelopes without modifying raw data; history list/detail return controlled errors instead of legacy fallback. | Server/client builds, archive verifier (80/80), Phase 3 verifier (25/25), history-routes verifier (23/23), final/facts/facts-recap/endstate verifiers (29/19/17/31), Postgres deletion check, client tests (13/13), and diff check passed: 237 automated checks, 0 failures. | Browser QA remains deferred. |
